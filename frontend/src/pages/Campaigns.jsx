...
import { Plus, Send, Users, TrendingUp, Trash2, Eye, Play, Pause, X, Paperclip, AlertTriangle, MessageSquare } from 'lucide-react'
import {
  getCampaigns,
  createCampaign,
  startCampaign,
...
function Campaigns() {
  ...
  const loadCampaigns = async () => {
    try {
      ...
      // Start polling for active campaigns, fetch progress once for paused
      response.data.forEach((c) => {
        if (c.status === 'active') startPolling(c.id)
        if (c.status === 'paused') {
          getCampaignProgress(c.id).then((res) => {
            setProgressMap((prev) => ({ ...prev, [c.id]: res.data }))
          }).catch(() => {})
        }
      })
    } catch (error) {
  ...
  const handleResume = async (id) => {
    try {
      ...
      alert('Failed to resume campaign: ' + (error.response?.data?.detail || error.message))
    }
  }
  ...
  const renderProgress = (campaign) => {
    ...
    if (!prog && campaign.status !== 'active' && campaign.status !== 'paused') return null
    ...
    return (
      <div className="w-full">
        ...
        <div className="text-xs text-gray-500 mt-1 text-right">
          {campaign.status === 'paused' && <span className="text-yellow-600 font-medium mr-1">Paused</span>}
          {sentPct}% complete
        </div>
      </div>
    )
              ...
              campaigns.map((campaign) => (
                <tr key={campaign.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 min-w-[200px]">
                    {campaign.status === 'active' || campaign.status === 'paused' || progressMap[campaign.id] ? (
